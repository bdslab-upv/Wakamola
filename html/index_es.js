...
function texto_vecinos(listado_vecinos, seleccionado) {
  ...
  document.getElementById("select_title_p").innerHTML = seleccionado.texto_es
  ...
  try {
    if (a.id != b.id) {
      if (relaciones_entre_nodos[a.id + "," + b.id]) {
        if ($.inArray(a.texto_es, listado_vecinos) == -1) { listado_vecinos.push(a.texto_es) }
        if ($.inArray(b.texto_es, listado_vecinos) == -1) { listado_vecinos.push(b.texto_es) }
